-- Allow practical private job-site videos while retaining tighter photo/document limits.
...
declare v_uid uuid:=auth.uid();v_row public.bct_project_files;v_existing public.bct_project_files;v_obj storage.objects;v_video boolean:=coalesce(p_mime_type,'') in ('video/mp4','video/quicktime');v_limit bigint;
begin
 ...
 v_limit:=case when v_video then 104857600 else 26214400 end;
