-- Project photos/PDFs remain 25 MB; job-site MP4/MOV videos may be up to 100 MB.
-- Storage bucket must allow the largest permitted project-file class; registration enforces per-type limits.
update storage.buckets set file_size_limit=104857600 where id='bct-project-files';
...
declare v_uid uuid:=auth.uid();v_row public.bct_project_files;v_existing public.bct_project_files;v_obj storage.objects;v_limit bigint;
begin
 ...
 v_limit:=case when p_mime_type in('video/mp4','video/quicktime') then 104857600 else 26214400 end;
 if p_file_size is not null and (p_file_size<0 or p_file_size>v_limit) then raise exception 'File size exceeds BCT limit for this file type';end if;
